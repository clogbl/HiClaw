# HiClaw: 基于 Kubernetes 声明式编排的协作式多 Agent 操作系统
...
- **通信权限声明式控制**：通过 `groupAllowFrom` 配置矩阵精确控制每个 Agent 接受谁的消息
...
每个 Worker 对应：一个 Docker 容器（或 K8s Pod）+ 一个 Matrix 通信账号 + 一个 MinIO 存储空间 + 一个 Gateway Consumer Token。
...
spec:
  ...
  workers:
    ...
    - name: bob
      ...
      runtime: copaw
      skills: [github-operations]
  peerMentions: true                  # 允许团队内 Worker 互相 @mention
```

Team 创建时，Controller 自动编排以下拓扑：

```
Leader Room:  Manager + Admin + Leader       ← Manager 与 Leader 的通信通道
Team Room:    Leader + Admin + W1 + W2 + ... ← Leader 与团队 Worker 的协作空间（Manager 不在）
Worker Room:  Leader + Admin + Worker        ← Leader 与单个 Worker 的私聊通道
Leader DM:    Admin + Leader                 ← Admin 与 Leader 的直接通道
```

关键：Team Room 不包含 Manager，实现了委派边界。
...
Manager 是可选组件。在纯声明式模式下，用户可以完全通过 `hiclaw apply` 管理资源，无需 Manager Agent。Manager 的价值在于提供自然语言交互入口——用户可以用对话的方式创建 Worker、组建 Team、分配任务。
...
hiclaw-controller 作为独立 Deployment 运行，Worker 作为 Pod 调度，基础设施组件（Tuwunel、MinIO、Higress）各自独立部署，支持水平扩展。
